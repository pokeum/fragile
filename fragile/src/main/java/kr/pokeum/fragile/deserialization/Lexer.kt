package kr.pokeum.fragile.deserialization
...
class Lexer(reader: Reader) {
    ...
    private val tokenMap = hashMapOf<Char, (Char) -> Token> (
        ',' to { c -> Token.COMMA },
        '{' to { c -> Token.LEFT_BRACE },
        '}' to { c -> Token.RIGHT_BRACE },
        '[' to { c -> Token.LEFT_BRACKET },
        ']' to { c -> Token.RIGHT_BRACKET },
        ':' to { c -> Token.COLON },
    ...
    private fun readNumberToken(firstChar: Char): Token {
        ...
        return Token.NumberValue(if (value.contains(".")) value.toDouble() else value.toLong())
    }
}